

// Ce fichier permet de faire toutes sortes de choses un peu magiques
// après qu'une page est chargé : il fait apparaître les modals,
// fait disparaître le menu, fait bouger tout seul des éléments, etc.

'use strict.js';



/**
* Import des modules
*/

// Import les fonctions
import * as fIsInViewport from './fonctions/_f-is-in-viewport.js';
import * as fIsSafari from './fonctions/_f-is-safari.js';

// Import les effets
import * as eTextScramble from './effets/_e-text-scramble';
import * as eTransition from './effets/_e-transition.js';
import * as eCameleon from './effets/_e-cameleon.js';
import * as eOneByOne from './effets/_e-one-by-one.js';
import * as eSlide from './effets/_e-slide.js';
import * as eDenko from './effets/_e-denko.js';
import * as eFloat from './effets/_e-float.js';
import * as eFade from './effets/_e-fade.js';

// Import les layouts
import * as lHeader from './layouts/_l-header.js';

// Import les modules
import * as mOverlay from './modules/_m-overlay.js';
import * as mSwiper from './modules/_m-swiper.js';
import * as mVolet from './modules/_m-volet.js';
import * as mOnglet from './modules/_m-onglet.js';
import * as mCover from './modules/_m-cover.js';
import * as mToast from './modules/_m-toast.js';
import * as mMenu from './modules/_m-menu.js';
import * as mBtn from './modules/_m-btn.js';
import * as mAccordions from './modules/_m-accordions.js';
import * as mInputChecker from './modules/_m-input-checker.js';
import * as mFormAutosave from './modules/_m-form-autosave.js';
import * as mFormChecker from './modules/_m-form-checker.js';
import * as mFormTextarea from './modules/_m-form-textarea.js';
import * as mFormNumber from './modules/_m-form-number.js';
import * as mFormSelect from './modules/_m-form-select.js';
import * as mFormLabel from './modules/_m-form-label.js';
import * as mFormPass from './modules/_m-form-pass.js';

// Import les fonctions de base
import * as bWindow from './base/_b-window.js';
import * as bClipboard from './base/_b-clipboard.js';
import * as bVar from './base/_b-var.js';

// Import les fonctions de Ajax
//import * as aAjaxExemple from './ajax/_a-ajax-exemple.js';




(() => {  //Début du IIFE
  document.addEventListener('DOMContentLoaded', () => {

    /**
    * Module : Onglet
    */

    let $ongletBtns = document.querySelectorAll('.m-onglet__btn');
    $ongletBtns.forEach($ongletBtn => {
      $ongletBtn.addEventListener('click', (e) => {
        let index = Array.from($ongletBtns).indexOf(this);

        let $elementsToDeactivate = document.querySelectorAll('.m-onglet__btn.e-on, .m-onglet__txt.e-on');
        $elementsToDeactivate.forEach(($element) => {
          $element.classList.remove('e-on');
        });

        this.classList.add('e-on');

        let $textElement = document.querySelector('.m-onglet__txt:nth-child(' + (index + 1) + ')');
        if ($textElement) {
          $textElement.classList.add('e-on');
        }
      });
    });


    /**
    * Effet is--FollowMouse
    */

    let $isFollows = document.querySelectorAll('.is--followmouse');

    if ($isFollows) {
      document.addEventListener('mousemove', (e) => {
        $isFollows.forEach(($isFollow) => {
          $isFollow.style.top = e.clientY + 28 + 'px.js';
          $isFollow.style.left = e.clientX + 'px.js';
        });
      });
    }
  });
})(); //Fin du IIFE